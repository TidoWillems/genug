...
BUILD_VERSION="${STAMP}${GIT_SHORT:+-$GIT_SHORT}"; export BUILD_VERSION

python3 generate_filelist.py; echo "✅ filelist.json erstellt"
PREV_LIST="tools/.build/md_list_prev.txt"; NEW_LIST="tools/.build/md_list_new.txt"; mkdir -p tools/.build
[[ -f "$PREV_LIST" ]] && cp "$PREV_LIST" "${PREV_LIST}.bak" || true
jq -r '.[]' filelist.json | sort > "$NEW_LIST"

./make_md_zip.sh; ./make_html_zip.sh
...
FILES_COUNT="$(jq 'length' filelist.json 2>/dev/null || wc -l < filelist.json)"
{
  ...
  if [[ -f "$PREV_LIST" ]]; then
    ...
    FILTERED=""
    for f in $NEW_FILES; do case "$f" in CHANGELOG.md|README.md|index.md) ;; *) FILTERED+="$f"$'\n';; esac; done
    if [[ -n "${FILTERED:-}" ]]; then
      echo "- Neu hinzugefügt:"; while IFS= read -r f; do [[ -n "$f" ]] && echo "  - $f"; done <<< "$FILTERED"
    fi
  fi
  echo
} >> CHANGELOG.md

cp "$NEW_LIST" "$PREV_LIST"; cp -f CHANGELOG.md "$STAGE/"
...
mirror -R --only-newer --parallel=2 --no-perms --no-symlinks \
  --exclude-glob '.git*' --exclude-glob '*.sh' --exclude-glob '*.py' \
  --exclude '.deploy.env' \
  "$STAGE" .
bye
EOF_LFTP
